# Memory Index

- [Project Overview](project_overview.md) — 5개 텔레그램 봇 통합 리포, GCP systemd 서비스 구조
- [Repo Consolidation 2026-04-01](project_repo_consolidation.md) — 분산 리포 5개를 단일 리포로 통합 완료
- [GCP Service Migration 2026-04-05](project_gcp_service_migration.md) — systemd 서비스 3개 통합 리포로 마이그레이션, 옛 디렉토리 삭제
- [User Profile](user_profile.md) — 텔레그램 봇 + Notion 자동화 개발자, 한국어 우선
- [GCP .env Loading](feedback_env_loading.md) — source 대신 export xargs 방식 사용 필요
- [GCP SSH from Local](reference_gcp_ssh.md) — gcloud CLI 설치 완료, gcp-ssh.cmd 래퍼로 원격 제어
